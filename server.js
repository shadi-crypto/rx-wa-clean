...
function adminHtml() {
  ...
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>RX WA — إدارة</title><style>body{font-family:Tahoma,'Segoe UI',sans-serif;background:#FBF7F0;padding:24px;max-width:860px;margin:auto;color:#222}input,select,textarea{padding:10px;margin:6px 0;width:100%;box-sizing:border-box;border:1px solid #ECE3D5;border-radius:8px;font-size:14px}button{background:#25D366;color:#fff;border:0;padding:11px 22px;border-radius:8px;cursor:pointer;font-weight:700;font-size:14px}.card{background:#fff;border:1px solid #ECE3D5;border-radius:14px;padding:20px;margin-bottom:18px}.card h3{margin:0 0 14px;color:#075E54;border-bottom:2px solid #ECE3D5;padding-bottom:8px}table{width:100%;border-collapse:collapse;margin-top:10px}td,th{border:1px solid #eee;padding:8px;text-align:right}.row{display:flex;gap:10px;flex-wrap:wrap}.row>*{flex:1;min-width:160px}.danger{background:#dc3545}.export{background:#0d6efd;display:inline-block;text-decoration:none}.info{font-size:13px;color:#777;margin-top:8px}</style></head><body><h1 style="color:#075E54">RX WA — لوحة الإدارة</h1>
  <div class="card"><h3>١) إضافة عميل جديد (ربط رقم واتساب + ذكاء اصطناعي)</h3><form method="POST" action="/admin/client"><div class="row"><input name="id" placeholder="معرف العميل (مثل store_a)" required><input name="name" placeholder="اسم المتجر" required></div><div class="row"><input name="phone_id" placeholder="Phone ID من ميتا" required><input name="wa_token" placeholder="WABA Token من ميتا" required></div><textarea name="system_prompt" placeholder="تعليمات الذكاء الاصطناعي (كيف يرد الموظف الآلي)" rows="3"></textarea><button>إضافة عميل</button></form></div>
